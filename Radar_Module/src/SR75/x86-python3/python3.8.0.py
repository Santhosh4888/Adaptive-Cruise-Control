...
CanDLLName = '/home/asl/Muni/SR75_Radar/USB-CAN-FD-B-Linux/VMware/x86-python3/libcontrolcanfd.so'
...
def main():
    ...
    # store the information of every object here
    obj_lst = []
    # Object attributes
    # ID - 0
    # XY - (X, Y)
    ...
    try:
        while True:
            raw_msgs = receive_can_data(dev_ch2)

            for i in range(len(raw_msgs)):
                # print(f"[{i}]:ts:{raw_msgs[i]['timestamp']}, id:{raw_msgs[i]['can_id']}, len:{raw_msgs[i]['dlc']}, "
                # f"eff:{raw_msgs[i]['eff']}, rtr:{raw_msgs[i]['rtr']}, "
                # f"data:{raw_msgs[i]['data']}")

                # Each frame data is embedded in 2 sub-frames
                # If you see a data frame with ID 0x701, grab the next frame as well
                # print()
                if(raw_msgs[i]['can_id'] == "0x600"):
                    num_objs = int(raw_msgs[i]['data'][0], 16) #convert hex to int
