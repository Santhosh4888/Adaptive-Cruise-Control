...
% Simulating a scenario where the controller sees the obstacle at a distance of 35m
% sensor and systems reaction combined takes whole 3sec and after that 
% vehicle starts decelerating as per requirement and stops at a safe distance from the obstacle.
% The simulation is done on point mass model. 


%% Parameters
v_lead = 0;                   % Lead vehicle/obstacle speed (m/s)
...
%% Simulation Loop
detected_time = -inf;  % Initialize detection time(-ve infinity)
...
for k = 1:n
    ...
    if d_rel < d_detection
        if detected_time == -inf  % Here it is set to negative infinity
