    ...
    def get_acceleration(self, ego_values, preceeding_values = None):
        ...
        if obs_pos:                                                       # If obstacle is there then CBF constraint is an essential constraint
            ...
            for k in range(self.Np):
                # v_target = min(self.desired_speed, obs_vel + 1.0)
                x_ref = np.array([0, obs_vel])
                S = np.array([[0, 0], [0, 1]])
                ...
                # Added acceleration smoothness penalty
                if k == 0:
                    cost += 50 * cp.square(u[0, k] - self.a_curr)
                else:
                    cost += 50 * cp.square(u[0, k] - u[0, k - 1])
        ...
        # -------- Safety check to avoid NoneType error --------
        # OSQP may fail or return infeasible results which leave u.value as None
        acc_val = u[0, 0].value
        if acc_val is None:
            # fall back to previous acceleration or zero and warn
            print("[Controller] warning: solver returned None, using previous acceleration")
            acc_val = self.a_curr
        self.a_curr = acc_val
        return acc_val
