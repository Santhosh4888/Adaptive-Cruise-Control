    ...
    def get_acceleration(self, ego_values, preceeding_values = None):
        ...
        if obs_pos:                                                       # If obstacle is there then CBF constraint is an essential constraint
            ...
            for k in range(self.Np):
                ...
                constraints += [CP.safety_factor * CP.a_min <= u[0, k], u[0, k] <= CP.safety_factor * CP.a_max] # Constraints on acceleration
                ...
                if k == 0:
                    constraints += [CP.safety_factor * CP.dec_Jerk_limit <= (u[0, k] - self.a_curr) / CP.sample_time, (u[0, k] - self.a_curr) / CP.sample_time <= CP.safety_factor * CP.acc_Jerk_limit]
                else:
                    constraints += [CP.safety_factor * CP.dec_Jerk_limit <= (u[0, k] - u[0, k - 1]) / CP.sample_time, (u[0, k] - u[0, k - 1]) / CP.sample_time <= CP.safety_factor * CP.acc_Jerk_limit]
        ...
        #if u[0, 0].value is not None:
            #self.a_curr = u[0, 0].value
        #else:
            #self.a_curr = CP.A * (0.0 - ego_vel)
        self.a_curr = u[0, 0].value
        return self.a_curr
