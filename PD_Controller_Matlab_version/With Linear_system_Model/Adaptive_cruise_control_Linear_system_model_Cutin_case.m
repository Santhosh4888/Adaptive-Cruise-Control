...
% at a very closer distance at speed lower the ego vehicle
% The car will be moving in speed mode until it finds the lead vehicle and 
% then transits into safe distance mode.
%% Here PD controller is used to test the algorithm on the system model in Cut-in Scenario.
clc;
clear all;
close all;
...
% Initial conditions
v_ego = 5.56;         % Initial speed of following vehicle (m/s)
...
for i = 1:n
    ...
    if d < d_detection %distance at which ego detects the lead vehicle
        ...
        x_ego  = x_ego +  v_ego * dt + (0.5 * A * (R_Vel-v_ego)* dt^2);
