...
//  nh.spinOnce();
  ...
  if (millis() - start_time <= test_time) {
    ...
    if (desired_voltage <= 0.5 && obs_detected) {
      // 
      analogWrite(A0, 0);
      nh.loginfo("Manual Braking activated");
      jrk.setTarget(690); // Actuator takes 4-5 sec to reach this position
      delay(3000);
      jrk.setTarget(2100);
    }
    else {
       // Actuator takes 4-5 sec to comeback to this position
