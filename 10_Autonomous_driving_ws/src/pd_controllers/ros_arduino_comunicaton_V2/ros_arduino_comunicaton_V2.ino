...
uint16_t const monitor_indices[] = {OD_VehSpeed, OD_MotorRPM, OD_ThrottlePot, OD_ThrottleCmd, OD_CurrentRMS, OD_RegenState, OD_VehDist, OD_VehAccel, OD_MasterTime};
float const monitorindices_scaling[] = {0.1, 1, 5.5/36044,100.0/32767,0.1, 1, 0.1, 10.0/10000, 0.1};
...
uint16_t const fast_params[] = {OD_VehSpeed, OD_MotorRPM, OD_ThrottlePot};
float const fast_scaling[] = {0.1, 1, 5.5/36044};
int const fastparams_length = sizeof(fast_params)/sizeof(fast_params[0]);
float fast_values[fastparams_length] = {};

uint16_t const slow_params[] = {OD_ThrottleCmd, OD_CurrentRMS, OD_RegenState, OD_VehDist, OD_VehAccel, OD_MasterTime};
float const slow_scalings[] = {100.0/32767,0.1, 1, 0.1, 10.0/10000, 0.1};
int const slowparams_length = sizeof(slow_params)/sizeof(slow_params[0]);
float slow_values[slowparams_length] = {};
...
//ros::Subscriber<std_msgs::Bool> brake_sub("brake_command", &brakeCallback);
...
void loop() {
  ...
  receive_sdo_upload_array(slow_params, slow_scalings, slow_values, 1);
  slow_params_index = (slow_params_index + 1) % 6;

//  send_sdo_upload(monitor_indices[5]);
//  monitor_index = (monitor_index) % monitorindices_length;
//  receive_sdo_upload_array(monitor_indices, monitorindices_scaling, monitorindices_values, monitorindices_length);
  ...
  if (!nh.connected()) {
    ...
    if (millis() - last_pub_time >= 100) {
      ...
      char speed_str[20];
      dtostrf(monitorindices_values[0], 7, 3, speed_str);
      
      char rpm_str[20];
      dtostrf(monitorindices_values[1], 9, 3, rpm_str);

      char log_msg[60];
      snprintf(log_msg, sizeof(log_msg), "Speed: %s km/h | RPM: %s | Throttle: %.3f", speed_str, rpm_str, monitorindices_values[2]);
      nh.loginfo(log_msg);

      
      vel_msg.data = monitorindices_values[0]; // VehSpeed
